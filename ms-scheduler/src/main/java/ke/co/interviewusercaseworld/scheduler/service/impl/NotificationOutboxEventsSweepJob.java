...
import ke.co.interviewusercaseworld.scheduler.repository.NotificationOutBoxEventRepository;
...
public class NotificationOutboxEventsSweepJob implements SweepJob {
    ...
    @Override
    @Job(name = "Notification Outbox Events", retries = 3)
    ...
    private Mono<NotificationOutboxEvent> markAsPublished(String requestRefId, NotificationOutboxEvent e) {

        Helpers.log(requestRefId, LogLevelEnum.info, OperationNameEnum.PUBLISH_OUTBOX_EVENTS_TASK, "marking outbox event as published", null);
        e.setPublished(true);
        return outboxEventRepository.save(e)
                .doOnError(throwable -> Helpers.log(requestRefId, LogLevelEnum.ERROR, OperationNameEnum.PUBLISH_OUTBOX_EVENTS_TASK, "Error occurred saving outbox event", new RuntimeException(throwable)))
                .doOnSuccess(res -> Helpers.log(requestRefId, LogLevelEnum.info, OperationNameEnum.PUBLISH_OUTBOX_EVENTS_TASK, "Saved outbox event: " + res.getId(), null));
    }
